...
\begin{example}\label{cmplx-thy-4}
	...
	As we have seen in \ref{cmplx-thy-3}, mod-$p$ the $p$-series is $\beta^{-\left(p-1\right)} x^p$, so that $v_1 = \beta^{-\left(p-1\right)}$ which is not a zero divisor $\Fp\left[\beta^{\pm 1}\right]$.
