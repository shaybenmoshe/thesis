\section{Overview of Chromatic Homotopy Theory}\label{sec:chromatic}
...
\begin{example}\label{chperf-tk}
	...
	Let $A \to B$ be a morphism between two complexes in $\mcl{T}$. Since tensor is a left adjoint, tensoring the cofiber with $K$ is given by
...
\begin{theorem}[{\cite[4.1.13]{Rav86}}]\label{mu-universal}
...
Denote by $\BP^*\left(X\right)$ the image of $\varepsilon^*$.
...
\begin{proposition}[{\cite[4.1.12 combined with A2.1.25 and A2.2.4]{Rav86}}]\label{bp-p-series}
	...
	$\left[p\right]\left(x\right) = \sum_F v_n x^{p^n}$ (note that the sum on the right hand side is in the formal group law).
\end{proposition}
...
\begin{proposition}\label{field-kunneth}
...
\begin{proof}
	...
	For $X,Y \in \Spfin_{\left(p\right)}$, by \cref{field-kunneth} we have 
	$$
	\K[n]_*\left(X \otimes  Y\right)
	= \K[n]_*\left(X\right) \otimes_{\K[n]_*} \K[n]_*\left(Y\right).
	$$
...
As such, it is a left adjoint (its right adjoint is the inclusion), and in particular preserves cofibers.
...
\begin{remark}
	...
	$$
		...
		\mcl{P}_{2, 1} \arrow[dash]{u}{} & \mcl{P}_{3, 1} \arrow[dash]{u}{} & \cdots & \mcl{P}_{p, 1} \arrow[dash]{u}{} & \cdots\\
...
The geometric points of the stack $\Mfgs$ are precisely the same as $\spec \SS$.
That is because for an algebraically closed field of characteristic $0$ there is a unique (up to strict isomorphism) formal group law which is of height $0$, namely the additive formal group law, and for characteristic $p$ there is a unique (up to strict isomorphism) formal group law of each height $1 \leq n \leq \infty$.
...
Similarly to $\MU$, $\BP_*$ is the universal ring with the universal $p$-typical formal group law, and $\left(\BP \otimes \BP\right)_*$ is the universal ring with two $p$-typical formal group laws and a strict isomorphism between them.
...
As we have seen, a complex orientation on a cohomology theory, which by \cref{mu-universal} is given by a map $\MU \to E$, has an associated formal group law, which is given by the map $L = \MU_* \to E_*$.
...
\begin{remark}
	Recall from \cref{bp-p-series} that for the formal group law over $\BP$ we have $\left[p\right]\left(x\right) = \sum_F v_n x^{p^n}$.
	Seemingly $v_n$ is a coefficient in a complicated sum involving $F$ itself.
	However, modulo $\left(v_0, v_1, \dotsc, v_{n-1}\right)$ (which is all we need for Landweber flatness) $v_n$ is equal to the coefficient of $x^{p^n}$ in the $p$-series expanded to a usual power series, which may be much easier to compute.
	To see this, first of all note that
	$
	\sum_F v_k x^{p^k}
	= \sum_{k \leq n, F} v_k x^{p^k} +_F \sum_{k > n, F} v_k x^{p^k}
	$.
...
We fix $k$ to be a perfect field of characteristic $p$ (i.e. the Frobenius is an isomorphism), and $\Gamma$ a formal group law over $k$ of height $n < \infty$.
...
\begin{definition}
	...
	A \emph{$\star$-isomorphism} between two deformations to $\left(R, \mfk{m}\right)$, $f: \left(G_1, i_1\right) \to \left(G_2, i_2\right)$, is defined only when $i_1 = i_2 = i$, and consists of an isomorphism $f: G_1 \to G_2$, such that $\pi^* f: i^*\Gamma = \pi^* G_1 \to \pi^* G_2 \to i^*\Gamma$ is the identity, i.e. $f\left(x\right) = x \mod \mfk{m}$.
...
The universal deformation can be chosen such that the formal group law $\Gamma_U$ over $\Wkui$ satisfies $\pi^* \Gamma_U = \Gamma$.
...
It is usually called Morava E-theory of height $n$, and denoted by $\mrm{E}_n$, when the field is $k = \mbb{F}_{p^n}$ and the formal group law is the Honda formal group law of height $n$.
