...
\begin{example}
	...
	Denote $S = R_\mfk{p}$, which is a local ring with maximal ideal $\mfk{m} = \mfk{p} R_\mfk{p}$ and residue field $k = S/\mfk{m}$, we are then reduced to showing that for $X = A \otimes R_\mfk{p}, Y = B \otimes R_\mfk{p}$ bounded complexes of finitely generated $S$-modules, such that $H_*\left(X \otimes_S Y\right) = 0$, either $H_*\left(X\right) = 0$ or $H_*\left(Y\right) = 0$.
	...
	Therefore, it suffices to prove that for $Z$, a bounded complex of finitely generated $S$-modules, $H_*\left(Z\right) = 0$ if and only if $H_*\left(Z/\mfk{m}\right) = 0$.
	
	For the first direction, if $Z$ is quasi-isomorphic to $0$, then the (derived) tensor product $Z \otimes_S k = Z/\mfk{m}$ is also quasi-isomorphic to $0$.
	
	For the other direction, assume that $Z/\mfk{m}$ is quasi-isomorphic to $0$.
	Recall that $Z$ is bounded, that is it can be written as
	$$
	Z = \cdots \to 0 \to Z_1 \xrightarrow{d_1} \cdots \xrightarrow{d_{n-2}} Z_{n-1} \xrightarrow{d_{n-1}} Z_n \to 0 \to \cdots,
	$$
	and that over a local ring, a finitely-generate projective module is free, therefore the modules $Z_i$ are free.
	...
	We are given that $H_*\left(Z/\mfk{m}\right) = 0$, so in particular it is exact at $Z_{n-1}/\mfk{m} \xrightarrow{d_{n-1}} Z_n/\mfk{m} \to 0$, i.e. $Z_{n-1}/\mfk{m} \xrightarrow{d_{n-1}} Z_n/\mfk{m}$ is surjective.
	That is, we know that $\im d_{n-1} = Z_n \mod \mfk{m}Z_n$, so by Nakayama's lemma we conclude that $\im d_{n-1} = Z_n$, i.e. $Z_{n-1} \xrightarrow{d_{n-1}} Z_n$ is surjective.
