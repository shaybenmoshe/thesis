...
\begin{example}\label{cmplx-thy-1}
	...
	We deduce that $\beta^{-1} t \in \K^2\left(\BU[1]\right)$ is mapped to $\beta^{-1} t \in \K^2\left(S^2\right) = \ZZ\left\{\beta^{-1} t\right\}$, which is mapped to $t \in \K^0\left(S^0\right) = \ZZ\left\{t\right\}$, which is indeed the generator of the reduced part.
...
\begin{example}\label{cmplx-thy-2}
	...
	We get that $x = \beta^{-1} t \mapsto \beta^{-1} u + \beta^{-1} v + \beta^{-1} u v = y + z + \beta y z = F_{\K}\left(y,z\right)$.
...
\begin{corollary}\label{k-e-p-series}
	The $p$-series associated to the formal group laws of $\K[n]$ and $\E{n}$ are $v_n x^{p^n}$ and $v_0 x + \dotsc v_n x^{p^n}$ respectively, and are therefore of height exactly $n$ and height $\leq n$ respectively.
...
$
are prime ideals.
Note that ${\HH{\mbb{Q}}}_*\left(X_{\left(p\right)}\right) = 0$ if and only if ${\HH{\mbb{Q}}}_*\left(X\right) = 0$ (this follows from the fact that rationalization and $p$-localization are both smashing, and the rational sphere is obtained from the $p$-local by inverting $p$), therefore
$
\mcl{P}_{p, 0}
= \left\{
	X \in \Spfin
...
\begin{theorem}[\todo{explain/reference}]
	...
	$,
	and the topology is such that the closed subsets finite unions of chains
...
\begin{example}\label{cmplx-thy-4}
	...
	Clearly $p = v_0$ is not a zero divisor in $\ZZ_p^\wedge\left[\beta^{\pm 1}\right]$.
	As we have seen in \ref{cmplx-thy-3}, mod-$p$ the $p$-series is $\beta^{p-1} x^p$, so that $v_1 = \beta^{p-1}$ which is not a zero divisor $\Fp\left[\beta^{\pm 1}\right]$.
...
\begin{example}\label{cmplx-thy-5}
	...
	= y + z + u y z
	$.
	It is then clear that the isomorphism $\ZZ_p^\wedge\left[u^{\pm1}\right] \to \ZZ_p^\wedge\left[\beta^{\pm1}\right]$, sends $u\Gamma_U$ to $F_{\K_p^\wedge}$.
	It follows that
	$
	\K_p^\wedge
