...
\begin{proof}
	...
	Since $F$ is exact, it sends cofibers to cofibers, so for $a,b \in F^*\mcl{P}$, i.e. $F\left(a\right), F\left(b\right) \in \mcl{P}$, and a map $a \to b$ we get
	$
...
Now, recall that $\Ch{R} \cong \Mod{\H	H{R}}$, therefore we can reinterpret the above theorem as $\spec{R} \cong \spec\left( \Mod{\HH{R}}^\mrm{comp} \right)$ (where the $\mrm{comp}$ denotes the compact objects in the category).
...
\begin{theorem}[{Quillen, \cite[4.1.6]{Rav86}}]
...
\begin{definition}
	Let $0 < n < \infty$.
	\emph{Morava K-theory} at height $n$ (and prime $p$, which is omitted in the notation), denoted by $\K[n]$, is the spectrum obtained by killing $p=v_0, \dotsc, v_{n-1}, v_{n+1}, \dotsc$ in $\BP$ and inverting $v_n$.
	Therefore $\K[n]_* = \mbb{F}_p\left[v_n^{\pm 1}\right]$.
	We also define $\K{0} = \HH{\mbb{Q}}$ and $\K{\infty} = \HH{\mbb{F}_p}$.
	Similarly, \emph{Morava E-theory} at height $n$ (and prime $p$), denoted by $\E{n}$, is the spectrum obtained by killing $v_{n+1}, v_{n+2}, \dotsc$ in $\BP$ and inverting $v_n$.
...
\subsection{The Balmer Spectrum $\spec \mbb{S}_{\left(p\right)}$ And $\spec \mbb{S}$}