...
\subsection{The Balmer Spectrum $\spec \SS_{\left(p\right)}$ And $\spec \SS$}
...
\begin{corollary}
	$\spec \SS_{\left(p\right)} = \left\{ \mcl{C}_{\geq 1}, \mcl{C}_{\geq 2}, \dotsc, \mcl{C}_{\geq \infty} \right\}$,
...
= \left\{
	X \in \Spfin
	\mid
	\K[n-1]_*\left(X_{\left(p\right)}\right) = 0
\right\}
$
and
$
\mcl{P}_{p, \geq \infty}
= L_{\left(p\right)}^* \mcl{C}_{p, \geq \infty}
= \left\{
	X \in \Spfin
	\mid
	X_{\left(p\right)} = 0
...
\begin{theorem}[\todo{explain/reference}]
	...
	\spec \SS
	= \left\{\Spfintor\right\}
	\cup \bigcup_p \left\{\mcl{P}_{p, \geq 2}, \dotsc, \mcl{P}_{p, \geq \infty}\right\}
	$,
	and the topology is such that the closed subsets finite unioins of chains
	$\left\{ \mcl{P}_{\geq k}, \mcl{P}_{\geq k+1}, \dotsc, \mcl{P}_{\geq \infty} \right\}$
	for some $k \geq 1$ (i.e. they may include $\Spfintor$).
	\todo{diagram}
\end{theorem}
\todo{regarding the topology, maybe I should prove that the pullback is also continous?}