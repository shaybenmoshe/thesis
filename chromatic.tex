...
\begin{example}
	...
	Let $A,B$ be complexes such that $A \otimes B \in \mcl{T}_{R_\mfk{p}}$, that is $A \otimes B \otimes R_\mfk{p}$ is quasi-isomorphic to $0$.
	...
	Denote $S = R_\mfk{p}$, which is a local ring with maximal ideal $\mfk{m} = \mfk{p} R_\mfk{p}$ and residue field $k = S/\mfk{m}$, and $X = A \otimes R_\mfk{p}, Y = B \otimes R_\mfk{p}$ are complexes of $S$-modules, and by the above $H_*\left(X \otimes_S Y\right) = 0$.
	In particular, $0 = H_*\left(X \otimes_S Y\right)/\mfk{m} = H_*\left(\left(X \otimes_S Y\right)/\mfk{m}\right) = H_*\left(X/\mfk{m} \otimes_k Y/\mfk{m}\right)$.
	The last term is the homology of a complex over the field $k$, so by K\"unneth, $0 = H_*\left(X/\mfk{m}\right) \otimes_k H_*\left(Y/\mfk{m}\right)$, from which it follows that one of them is $0$, and without loss of generality we assume $0 = H_*\left(X/\mfk{m}\right) = H_*\left(X\right)/\mfk{m}$.
	Now, recall that we consider perfect complexes (and that $- \otimes R_\mfk{p}$ preserves that property), so $X$ is quasi-isomorphic to a bounded complex of finitely-generated projective $S$-modules $X'$, so $H_*\left(X\right) \cong H_*\left(X'\right)$.
	In particular, $0 = H_*\left(X'\right)/\mfk{m}$.
	Furthermore, a projective module over a local ring is free, so the modules in $X'$ are finitely-generated free modules, which implies that each $H_i\left(X'\right)$ is a finitely-generated free module (because it is a quotient of such a module).
	We know that $0 = H_i\left(X'\right)/\mfk{m}$, and so Nakayama's lemma implies that $0 = H_i\left(X'\right) \cong H_i\left(X\right) = H_i\left(A \otimes R_\mfk{p}\right)$, and the conclusion follows.
\end{example}
