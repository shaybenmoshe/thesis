...
\begin{theorem}[Thick Subcategory Theorem \cite{HS}]\label{thick-subcategory-thm}
...
$$
are prime ideals.
Note that ${\HH{\mbb{Q}}}_*\left(X_{\left(p\right)}\right) = 0$ if and only if ${\HH{\mbb{Q}}}_*\left(X\right) = 0$ (this follows from the fact that rationalization and $p$-localization are both smashing, and the rational sphere is obtained from the $p$-local by inverting $p$), hence
$
\mcl{P}_{p, 0}
= \left\{
	X \in \Spfin
