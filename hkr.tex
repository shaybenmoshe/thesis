...
\begin{proposition}
	The rings $L_r$ have an $\aut\left(\Lambda\right)$-action, compatible with the maps $L_r \to L_{r+1}$.
...
\begin{definition}
	...
	$$
	This algebra still has an $\aut \left(\Lambda_r\right)$-action.
\end{definition}
...
We wish to give an alternative description of the algebra before taking the $G$ fixed points.
Simply by taking the coproduct as a product out of the cohomology, and out of the tensor product, we get:
...
To that end, we first formulate a very general combinatorial statement:
...
We now want to apply this to our case, to give a combinatorial description of $\cl[n,p]{G,X; L_r}^{\aut \left(\Lambda_r\right)}$.
...
\begin{proof}
	...
	It is then easy to see that by definition these maps are compatible with the maps $L_r \to L_{r+1}$, coming from the projections.
\end{proof}
