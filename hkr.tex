...
$$
\begin{tikzcd}
	E^*\formal{x} \arrow{d}{x \mapsto x} & E^*\formal{x} \arrow{d}{x \mapsto x} \arrow{l}{x \mapsto [k]\left(x\right)} \\
	E^*\formal{x}/\left(\left[m\right]\left(x\right)\right) & E^*\formal{x}/\left(\left[l\right]\left(x\right)\right) \arrow{l}{}
\end{tikzcd}
$$
We then see that the bottom map is also given by $x \mapsto [k]\left(x\right)$.
\todo{does it really follow?}
...
It is not too hard to define the rings $L_r = L_r\left(E^*\right)$ and $L = L\left(E^*\right)$ which will be used, but it is harder to show their properties.
We will give a quick definition, and claim, first without proving, the properties we need.
...
By post composing with the localization map, we get $E^*\left(\BB[\Lambda_r]\right) \to L_{r+1}$.
