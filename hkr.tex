...
\begin{itemize}
	...
	\item Use Kunneth and multiplicativity to reduce to $\left(\ZZ/{q^t}, *\right)$,
	\item Prove for $\left(\ZZ/{q^t}, *\right)$.
\end{itemize}

We move on to introducing the idea of complex oriented descent.
We will need some definitions before that.
...
\begin{proposition}
	...
	Then the $G$-space $F\left(X \times V\right) = X \times F\left(V\right)$ has abelian stabilizer.
\end{proposition}

\begin{proof}
	Let $\left(x, \left(\ell_i\right)\right)$ be a point in $X \times F\left(V\right)$.
	We wish to show that its stabilizer is abelian.
	Let $g,h \in G$ be two elements which fix the point, that is, $g.x = x = h.x$ and $\rho\left(g\right)\left(\ell_i\right) = \ell_i = \rho\left(h\right)\left(\ell_i\right)$.
	We see that the linear transformations $\rho\left(g\right): V \to V$ and $\rho\left(h\right): V \to V$ are simultaneously diagonalizable w.r.t to the decomposition $\left(\ell_i\right)$ of $V$.
	Therefore, by a classical result in linear algebra, they commute, $\rho\left(g\right) \rho\left(h\right) = \rho\left(h\right) \rho\left(g\right)$, i.e. $\rho\left(gh\right) = \rho\left(hg\right)$.
