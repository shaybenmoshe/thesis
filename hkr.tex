...
Similarly to the proof Atiyah-Segal theorem, the actual proof of Theorem C involves a general construction, even to prove the specific case we are interested in, but it will be easier to state it first for the specific case.
...
We will first define some of the objects above, to make things more precise, and we will see what exactly we need to construct everything.
...
\begin{proposition}[{\cite[5.8]{HKR}}]
	...
	Write $m = sp^t$ with $s$ coprime to $p$, then this is a free $E^*$-module of rank $p^{nt}$ (where $n$ was the height of $\Gamma$).
\end{proposition}
...
Since $\Lambda_r$ acts on itself by left multiplication, by functoriality we get that it also acts on $E^*\left(\BB[\Lambda_r]\right)$.
...
\begin{remark}[Algebro-geometric interpretation of $L_r$]\label{alg-geo-Lr}
	...
	By definition, the $p^r$-torsion elements in $\fG$, is the scheme-theoretic kernel of the multiplication-by-$p^r$ map $\left[p^r\right]: \fG \to \fG$, that is $\fG\left[p^r\right] = \spec\left(R\formal{t}/\left(\left[p^r\right]\left(t\right)\right)\right) = \spec E^0\left(\BB[\ZZ / p^r]\right)$.
	Since $\Gamma_U$ is of height $n$, the leading term of the $p^r$-series, is $t^{p^{rn}}$, which by a variant of Weierstrass preparation, see \cite[5.1]{HKR}, shows that $\fG\left[p^r\right]$ is of degree $p^{rn}$.
...
\begin{example}[K-Theory Saga: The Ring $L_r$]\label{k-thy-Lr}
	...
	We note that this is in accordance with the algebro-geometric point of view from \ref{alg-geo-Lr}, as the this is of degree $p^r$ over $\ZZ_p$, and the points are isomorphic to the group of roots of unity of order $p^r$.
	\todo{how to state this better?}
	...
	Since the codomain is a field, this is invertible, so by the universal property of localization, we indeed get a map ${S_r}^{-1} R \to \QQ_p\left[s\right]/\left(\Phi_{p^r}\right)$.
	
	Second, look at the map $\QQ_p\left[s\right] \to {S_r}^{-1} R$ (the composition of the quotient and localization maps).
	Recall that $s^{p^r} - 1 = \left(s^{p^{r-1}} - 1\right) \Phi_{p^r}\left(s\right)$, which is the element we took the quotient by.
	Further, we inverted $\left(s^{p^{r-1}} - 1\right)$, so by multiplying by its inverse, we also get that $\Phi_{p^r}\left(s\right)$ is zero in ${S_r}^{-1} R$.
...
This is well defined, since if $x$ fixed by $\alpha$, i.e. $g_i x = x$, which implies that $\gamma g_i \gamma^{-1} \gamma x = \gamma x$, so $\gamma x$ is fixed by $\gamma. \alpha$.
Moreover, it admits an $\aut\left(\Lambda_r\right)$-action.
Let $\varphi \in \aut\left(\Lambda_r\right)$, for any $\alpha \in \Gnp$, clearly $\im \alpha = \im \left(\alpha\circ\varphi\right)$, so $x \in X^{\im \alpha}$ is mapped by $\varphi$ to $x \in X^{\im \left(\alpha\circ\varphi\right)}$ (i.e. this action just "shuffles" to coordinates labeled by the $\alpha \in \Gnp$).
...
\begin{proof}
	...
	When $X = *$, all fixed points $X^{\im \alpha}$ are again trivial, which shows that the $E^*$-cohomology is simply $E^*$, and tensoring with it over $E_*$ does nothing, and th specific case follows.
\end{proof}
...
Since $p$ is invertible in $L$ by \ref{Lr-fixed-points}, and $\aut \left(\Lambda\right)$ doesn't change $p^{-1}$, it also follows that after inverting $p$, that is, rationalizing, the map $p^{-1} \chinpG: p^{-1} E^*\left(\EG \times_G X\right) \to \cl[n,p]{G,X; L}$ still lands in the $\aut \left(\Lambda\right)$ fixed points (and the same is true for $L_r$ in place for $L$).
...
\begin{proof}
	...
	Note that in general if $B$ is an $A$ algebra, then $\hom_A\left(A, B\right) \cong \hom_B\left(A \otimes_A B, B\right)$, in one direction the map $f$ is mapped to the map $a \otimes b \mapsto f\left(a\right)b$, and in the other direction $g$ is mapped to $a \mapsto g\left(a \otimes 1\right)$.
	Using the exponential rule and the remark above, we get that the character map is the same data as a map
	$\phi: \Lambda_k \to \hom_{E^*}\left(E^*\formal{x}/\left(\left[p^k\right]\left(x\right)\right), L_r\right)$.
	
	Recall the definition of $L_r$ from \ref{Lr}, and the definition of the character map.
	Chasing the definitions, we that for $0 \neq \left(l_i\right) \in \Lambda_k$, $\phi\left(\left(l_i\right)\right)$ is the homomorphism which sends $x$ to $\sum_F \left[l_i\right]\left(x_i\right) \in L_r$.
