\section{Atiyah-Segal}\label{sec:AS}
...
We should also note that beyond this section, we will be mostly interested in finite groups.
...
In every step they show that the theorem holds for a more general type of group:
\begin{itemize}
...
We note that the first formulation of the Atiyah-Segal theorem \ref{AS-private}, is indeed a private case of the second formulation \cref{AS-full}.
Take the case $X = *$.
By definition, $\KG^*\left(*\right)$ is finite over $\RgG = \KG^*\left(*\right)$, so \cref{AS-full} holds and we have an isomorphism of pro-rings $\left\{\KG^*\left(*\right)/\left(I^*\right)^n\right\}_n \to \left\{\KG^*\left(\EG_n\right)\right\}_n$.
...
In \cref{k-thy-oriented} we denoted $t = L-1$ (warning: there we looked at $\K^*$, now we focus on $\K$), which allows us to rewrite this as $\K\left(\CP{n}\right) \cong \ZZ\left[t\right] / t^{n+1}$.
...
This shows that the map admits $\K\left(\BU[1]\right) \cong \ZZ\formal{t}$ as the $I = \left(t\right)$-completion of $\RG[\UU[1]]$.
...
Similarly to before, the augmentation $\varepsilon: \RG[\ZZ/2] \to \ZZ$ sends $1$ and $\rho$ to $1$, so clearly $\left(\rho-1\right) \subseteq I$, and for the same reasoning as in the previous example this is actually an equality.
...
Since the forgetful functor from rings to abelian groups is a right adjoint, it commutes with limits, so this will give us the abelian group structure.
...
To conclude, \cref{char-1} tells us that $\RG \otimes \Qab \cong \cl{G; \Qab}$, and \cref{char-2} tells us that $\RG \otimes \QQ \cong \cl{G; \Qab}^{\Gal\left(\Qab / \QQ\right)}$.
