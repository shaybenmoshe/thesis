...
Note that $\K_G$ is no longer a homotopy invariant, since it also takes into account the action of $G$.
...
All of these are $\RgG$-modules, and they show that this map factors through the quotient by $\left(I^*\right)^n$, to give a map $\KG^*\left(X\right)/\left(I^*\right)^n \to \KG^*\left(X \times \EG_n\right)$.
...
It is known that the irreducible representations are of dimension $1$ and labeled by an integer $m \in \ZZ$, i.e. are homomorphisms $\rho_m = \UU[1] \to \UU[1]$ given by $\rho_m\left(e^{i \theta}\right) = e^{m i \theta}$.
...
Therefore the representation ring generated under (virtual) direct sums and tensor products from $\rho_1$ and $\rho_{-1}$.
Moreover, $\rho_1 \otimes \rho_{-1} = 1$.
Therefore we conclude that $\RG[\UU[1]] = \ZZ\left[\rho, \rho^{-1}\right]$.

The augmentation is the homomorphism $\varepsilon: \RG[\UU[1]] \to \ZZ$, which sends $1,\rho,\rho^{-1}$ to $1$.
Recall that the augmentation ideal is $I = \ker \varepsilon$.
Denote $t = \rho-1$, which clearly belongs to $I$.
We can also write then $\RG[\UU[1]] = \ZZ\left[t, \left(1+t\right)^{-1}\right]$.
Note that $\varepsilon$ factors to a map $\RG[\UU[1]]/\left(t\right) \to \ZZ$, which is already an isomorphism, so from the first isomorphism theorem indeed $I = \left(t\right)$.
...
Similarly to before, the augmentation $\varepsilon: \RG[\ZZ/2] \to \ZZ$ sends $1,\rho$ to $1$, so clearly $\left(\rho-1\right) \leq I$, and for the same reasoning as in the previous example this is actually an equality.
